...
class TrendingTopicImportFormView(LoginRequiredMixin, ScitweetsContextMixin, FormView):
    ...
    def parse_trending_topic_data_from_text_file(self, text_file):
        ...
        if tt_imported:
            if not tt_found:
                success_text = u"%(imported)d trending topics were imported from file" % imported_tts
            else:
                success_text = u'%(imported)d trending topics were imported from file and ' \
                               u'%(existing)d already existed in the database' % imported_tts
            messages.success(self.request, ugettext_lazy(success_text))
        else:
            warning_text = u'No new trending topics were created.'

        if tt_failure_format:
            failure_text.append(u'There were %(failure_format)d trending topics had format errors' % imported_tts)
        if tt_failure_other:
            failure_text.append(u'%(failure_other)s trending topics led to unexpected errors' % imported_tts)
    ...
    def form_valid(self, form):
        ...
        imported_trendingtopics = self.parse_trending_topic_data_from_text_file(form_file.file)

        if not imported_trendingtopics:
            return super(TrendingTopicImportFormView, self).form_invalid(form)

        return super(TrendingTopicImportFormView, self).form_valid(form)
